...
static const unsigned char PROGMEM DolphinFlip[] = {0x10,0x40,0x00,0x00,0x00,0x00,0x10,0x40,0x00,
 ...
 void drawMainMenu() {  
    ...
    tft.setTextSize(3);
    tft.setFreeFont();
    tft.drawString("DEMO MENU", 45, 30);
...
void Buzzbeep() {
    tone(buzzPin, 3000, 255); //Hz
    buzzerStartTime = millis(); 
    buzzerActive = true; 
}

void updateBuzz() {
    if (buzzerActive && millis() - buzzerStartTime >= 10) {
        noTone(buzzPin); 
        buzzerActive = false; 
    }
}
